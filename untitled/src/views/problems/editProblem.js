...
const EditProblem = () => {
    ...
    useEffect(() => {
        ...
        // Fetch the problem details from the server based on the problem ID
        // and populate the state variables with the fetched data
        fetch(`http://121.36.214.230:9000/toUpdate?id=${problemId}`,requestOptions)
            ...
            .then((data) => {
                ...
                setTest_in(data.test_ins);
                setTag(data.tag);
                setTest_out(data.test_outs);
                setInput_desc(data.input_desc);
                setOutput_desc(data.output_desc);
                setTextAreas(data.test_ins.map((testIn, index) => ({ id: index + 1, value: testIn })));
                setResultAreas(data.test_outs.map((testOut, index) => ({ id: index + 1, value: testOut })));
    ...
    const handleSubmit = (event) => {
        ...
        // Example using fetch:
        fetch(`http://121.36.214.230:9000/update`, {
            method: "POST",
            headers: {
